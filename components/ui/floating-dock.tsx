...
export const FloatingDock = ({ items, desktopClassName, mobileClassName }: { items: DockItem[]; desktopClassName?: string; mobileClassName?: string }) => {
  return (
    <>
      <FloatingDockDesktop items={items} className={desktopClassName} />
      <FloatingDockMobile items={items} className={mobileClassName} />
...
const FloatingDockMobile = ({ items, className }: { items: DockItem[]; className?: string }) => {
  ...
  return (
    <div className={cn("relative block md:hidden", className)}>
      <AnimatePresence>
        {open && (
          <motion.div layoutId="dock-mobile" className="absolute inset-x-0 bottom-full mb-3 flex flex-col gap-2">
            {items.map((item, idx) => (
              <motion.a
                key={item.title}
                href={item.href}
...
const FloatingDockDesktop = ({ items, className }: { items: DockItem[]; className?: string }) => {
  ...
  return (
    <motion.div onMouseMove={(e) => mouseX.set(e.pageX)} onMouseLeave={() => mouseX.set(Infinity)} className={cn("mx-auto hidden h-16 items-end gap-4 rounded-2xl px-4 pb-3 md:flex", className)}>
      {items.map((item) => (
...
function IconContainer({ mouseX, title, icon, href }: { mouseX: MotionValue; title: string; icon: React.ReactNode; href: string }) {
  ...
  const size = useSpring(useTransform(distance, [-160, 0, 160], [40, 72, 40]), { stiffness: 220, damping: 20, mass: 0.4 });

  const iconSize = useSpring(useTransform(distance, [-160, 0, 160], [18, 36, 18]), { stiffness: 220, damping: 20, mass: 0.4 });
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="
                absolute -top-9 left-1/2 whitespace-nowrap
